*&---------------------------------------------------------------------*
* 모듈/서브모듈   : SD/SDC
* Program ID : ZR19A00040
* Desc       : DO(납품)로->BI(청구) 생성하기
* Transaction: ZR19A00040
* Creator    : REM0019
...
DATA: gt_zlikp19 TYPE TABLE OF ZLIKP19, "ZLIKP19(H) + bi문서번호
      gt_zlips19 TYPE TABLE OF ZLIPS19, "ZLIPS19(I)
...
*~do-bi link(중복확인)~
DATA: gt_crbi TYPE TABLE OF ZVBRP19 WITH NON-UNIQUE SORTED KEY idx01
      COMPONENTS VGBEL.
...
INITIALIZATION.
CLEAR gv_display_60.
IMPORT gv_display_60 FROM MEMORY ID 'DISPLAY_DO'.
FREE MEMORY ID 'DISPLAY_DO'.
...
FORM 1000_onli .
  ...
  REFRESH: gt_zlikp19, gt_zlips19, gt_crbi.
...
*~2-2. ZVBRP19~
    ...
    LOOP AT lt_zlikp19 ASSIGNING FIELD-SYMBOL(<ls_zlikp19>).
      CHECK <ls_zlikp19>-VBELN IS NOT INITIAL.
      APPEND INITIAL LINE TO lt_zvbrp19 ASSIGNING FIELD-SYMBOL(<ls_zvbrp19>).
      <ls_zvbrp19>-VGBEL = <ls_zlikp19>-VBELN. "DO->BI
    ENDLOOP.
...
*--------------------
*~~3. gt_crbi~~
* gt_crbi에 중복제거한 VGBEL(참조됨)과, VBELN넣는다 -> PRECHECK에서 검증할것!
*--------------------
    SORT lt_zvbrp19 BY VGBEL.
    DELETE ADJACENT DUPLICATES FROM lt_zvbrp19 COMPARING VGBEL.

    IF lt_zvbrp19 IS NOT INITIAL.
      SELECT VBELN VGBEL
        INTO CORRESPONDING FIELDS OF TABLE gt_crbi
      FROM ZVBRP19
        FOR ALL ENTRIES IN lt_zvbrp19
      WHERE VGBEL = lt_zvbrp19-VGBEL.
    ENDIF.
...
*& 헤더에서 특정 CELL(VBELN) 클릭하면 gt_zlikp19_t에서 그 cell의 아이템만 걸러줌!
*&---------------------------------------------------------------------*
FORM av2000_1_cell_click
  USING pv_av_name
        pc_gubun
...
*&---------------------------------------------------------------------*
*& create SO 버튼(zlea_crdo = CRBI)을 클릭할 때 실행되는 이벤트 처리
*&---------------------------------------------------------------------*
*& -->  p1        text
*& <--  p2        text
*&---------------------------------------------------------------------*
...
* 1개건만 진행
  ...
  LOOP AT pt_rows ASSIGNING FIELD-SYMBOL(<ls_rows>).
      ...
      IF sy-subrc = 0.
        ...
        "8-2 : 실행시 실행할 ITEM이 없으면 LINK정보를 담아줄수 없으므로 오류처리
         READ TABLE gt_zlips19_t TRANSPORTING NO FIELDS WITH TABLE KEY idx01
           COMPONENTS VBELN = <ls_zlikp19>-VBELN.
           IF sy-subrc <> 0.
             pv_error = zlea_x.
...
*&---------------------------------------------------------------------*
*       사용자 최종 확인 팝업(정말 진행하시겠습니까?) - GOTO->TEXT ELEMENTS 넣어야 됨
*----------------------------------------------------------------------*
*      -->P_PV_UCOMM  text
*      <--P_PV_ERROR  text
*----------------------------------------------------------------------*
...
FORM zz_set_sel_rows_ucomm  USING    pv_ucomm
  ...
  DATA: lt_lips TYPE ZTTLIPS19.

  lt_lips = gt_zlips19_t.

  DELETE lt_lips WHERE VBELN <> ps_likp-VBELN.
...
FORM zz_get_selrows_precheck  USING    ps_likp LIKE LINE OF gt_zlikp19
  ...
  READ TABLE gt_crbi TRANSPORTING NO FIELDS
