*&---------------------------------------------------------------------*
* 모듈/서브모듈   : SD/SDC
* Program ID : ZR19A00010
* Desc       : Create PO -> SO
* Transaction: ZR1900080
* Creator    : REM0019
...
TABLES: ekko, ekpo.
DATA: gt_ekko   TYPE TABLE OF zekko19. "EKKO
DATA: gt_ekpo_t TYPE TABLE OF zekpo19 WITH NON-UNIQUE SORTED KEY idx01 "헤더에서 하나 선택 시 거기에 해당하는 아이템만 모아서 보여주기 위해 만든 테이블
        COMPONENTS ebeln.
DATA: gt_ekpo   TYPE TABLE OF zekpo19.

* PO_SO Link
DATA: gt_poso TYPE TABLE OF zvbak19 WITH NON-UNIQUE SORTED KEY idx01
        COMPONENTS bstnk.
...
*  DATA: lt_ekpov TYPE SORTED TABLE OF ekpo WITH UNIQUE KEY ebeln.
  DATA: lt_ekpov TYPE TABLE OF zekko19.
  DATA: lt_ekpo  TYPE TABLE OF ekpo WITH NON-UNIQUE SORTED KEY idx01
        COMPONENTS ebeln.

  DATA: lt_vbak19v TYPE TABLE OF zvbak19.
...
***      WHERE bstnk+0(10) = lt_ekpov-ebeln.
    ...
    LOOP AT lt_ekpov ASSIGNING FIELD-SYMBOL(<ls_ekpov>).
      ...
      APPEND INITIAL LINE TO lt_vbak19v ASSIGNING
      FIELD-SYMBOL(<ls_vbak19v>).
      <ls_vbak19v>-bstnk = <ls_ekpov>-ebeln.
    ENDLOOP.

    SORT lt_vbak19v BY bstnk.
    DELETE ADJACENT DUPLICATES FROM lt_vbak19v COMPARING bstnk.
    IF lt_vbak19v IS NOT INITIAL.
      SELECT bstnk vbeln
        INTO CORRESPONDING FIELDS OF TABLE gt_poso
        FROM zvbak19
        FOR ALL ENTRIES IN lt_vbak19v
        WHERE bstnk = lt_vbak19v-bstnk.
    ENDIF.
...
*&      Form  1000_AFTE
*&---------------------------------------------------------------------*
*       처음 실행했을 때 HEADER TABLE의 엔트리가 몇 건 조회되었는지 띄워 준다!
*----------------------------------------------------------------------*
*  -->  p1        text
*  <--  p2        text
*----------------------------------------------------------------------*
  ...
  PERFORM 0o_av_make
   TABLES gt_ekko
    USING 'GO_AV2000_1' go_ic2000_1
          'ZEKKO19'   " 아밥 딕셔너리에 선언된 테이블, 구조, 뷰를
*                    이용해서 필드카테고리를 정의할 수 있다.
          ''.

  0o_av_refresh 'GO_AV2000_1' 'X' 'X' 'X'.        "x: 열과 행을 유지해라!

  "테이블
  PERFORM 0o_av_make
   TABLES gt_ekpo
    USING 'GO_AV2000_2' go_ic2000_2
          'ZEKPO19' " 프로그램 내의 내부테이블을
*                      이용해서 필드카테고리를 정의할 수 있다.
          ''.
...
*&---------------------------------------------------------------------*
*& create SO 버튼(zlea_crso)을 클릭할 때 실행되는 이벤트 처리
*&---------------------------------------------------------------------*
*& -->  p1        text
*& <--  p2        text
*&---------------------------------------------------------------------*
...
*&---------------------------------------------------------------------*
*& CREATE SO를 눌렀을 때 선택 상태가 올바른지 검증! (유효성 검사)
*&---------------------------------------------------------------------*
*&      --> LT_ROWS
*&      --> ZLEA_CRSO
*&      <-- LV_ERROR
...
* 1개건만 진행
  ...
  LOOP AT pt_rows ASSIGNING FIELD-SYMBOL(<ls_rows>).
    ...
    IF sy-subrc = 0.
      CASE pv_ucomm.
        WHEN 'CRSO'.
          PERFORM zz_get_selrows_precheck USING <ls_ekko> CHANGING pv_error.
      ENDCASE.
    ENDIF.
